...
**Status:** A0 and A0.1 are now **APPLIED AND VERIFIED IN PRODUCTION** (`yisxnwbsnzxjnmzpstzj`, staged security migration promotion pass, 2026-09-09). This document originally described read-only re-verification + proposal revision only — that content is preserved below (§1–14) as the accurate record of what was found before promotion. This status block records what actually happened when the proposals were applied.

**A0 — `supabase/migrations/20260909035504_rls_hardening_dna_import_spotify_apple.sql`**: **APPLIED + VERIFIED.** All four policy changes (dna_records, import_logs, artist_spotify_snapshots write, apple_metrics_daily) took effect exactly as designed on the first application — no surprises, no correction needed. Live-reverified afterward: `dna_records`/`import_logs`/`apple_metrics_daily` anon access closed; `artist_spotify_snapshots`'s public SELECT preserved exactly (confirmed via a session-less `SET ROLE anon` query returning without a permission error, matching the real `lib/cms/index.ts` client's behavior); `service_role.rolbypassrls = true` confirmed directly, guaranteeing every service-role-backed admin path was unaffected.

**A0.1 — two migrations, not one:**
- `supabase/migrations/20260909035736_append_inbox_log_hardening.sql`: **APPLIED — PARTIAL HARDENING.** `search_path` pin took effect correctly. The `REVOKE EXECUTE ... FROM anon, authenticated` statements executed without error but **did not close the vulnerability** — live verification immediately after showed `has_function_privilege('anon', ..., 'EXECUTE')` still `true`.
- **Root cause, discovered via `aclexplode(pg_proc.proacl)`:** PostgreSQL grants `EXECUTE` on every newly created function to the `PUBLIC` pseudo-role by default unless explicitly revoked at creation time. `append_inbox_log`'s ACL had a `PUBLIC: EXECUTE` entry that the original hardening proposal never accounted for — revoking from `anon`/`authenticated` individually removed their own ACL entries but left the `PUBLIC` grant standing, and every role implicitly has whatever `PUBLIC` grants, with no way to opt out short of revoking from `PUBLIC` itself.
- `supabase/migrations/20260909040002_append_inbox_log_revoke_public.sql`: **APPLIED + VERIFIED.** `REVOKE EXECUTE ... FROM PUBLIC`. Verified via all 10 conditions the founder specified: `PUBLIC`/`anon`/`authenticated` effective EXECUTE all `false`; `service_role`/`postgres` EXECUTE both still `true` (independent ACL entries, unaffected); function still `SECURITY DEFINER`; `search_path` still pinned; `audio_inbox`'s own RLS policy unchanged; the legitimate caller's static auth/client path unaffected; zero rows touched (`audio_inbox` row count unchanged at 0, and ACL/GRANT statements never touch row data structurally).

**Corrected finding this pass:** prior docs (`SUMG_PRODUCTION_SCHEMA_DRIFT_AUDIT.md`, `SUMG_MASTER_VAULT_PRODUCTION_READINESS.md`) characterized production as "almost entirely empty" based on `list_tables`'s `rows` field. That field is a stale `pg_class.reltuples`-style estimate, not a live count — direct `COUNT(*)` queries during this pass found `dna_records` has 9 rows, `import_logs` has 5, `songs`/`releases` have 32 each, `artists` has 9, `assets` has 25. Production has real catalog metadata, not zero rows. The one specific claim that mattered most for risk framing — "0 songs have a non-null `audio_url`" — was re-verified with a real `COUNT(*)` and still holds (it was never based on the stale statistic). This means the RLS gaps just closed on `dna_records`/`import_logs` were exposing real rows to `anon`, not a theoretical empty-table risk — the fix was more consequential than earlier framing suggested. See `SUMG_MIGRATION_DRY_RUN_IMPACT.md` for the corrected note.
