...
**UPDATE 2 (same day, follow-up pass): `artist_spotify_snapshots`'s public SELECT — deliberately left open by A0 — is now also closed.** `supabase/migrations/20260909041713_artist_spotify_snapshots_select_hardening.sql`, applied and verified. This one required an application-code change first (moving the two Spotify-snapshot functions in `lib/cms/index.ts` to a new `lib/cms/admin-spotify.ts`, using the session-aware server client instead of a session-less anon client) — the migration itself was pure policy DDL, ROWS TOUCHED = 0, exactly as this doc's methodology would have predicted had A0 included it. See `SUMG_SECURITY_MIGRATION_HARDENING.md`'s new section for the full account, including how the "0 rows in the table" fact meant verification had to prove the `is_cms_role()` predicate directly rather than infer allow/deny from row-count side effects.
