...
/**
 * Streaming / publishing platforms that generate royalty earnings.
 * "Other" covers any platform not listed here (add manually via notes).
 */
export type RoyaltyPlatform =
  | "Spotify"
  | "Apple Music"
  | "YouTube Music"
  | "Amazon Music"
...
/**
 * A single row of royalty earnings for one song/release on one platform
 * for a given calendar month.  Gross amount before any splits are applied.
 */
export interface RoyaltyEarning {
  id: string;
  /** Slug of the song these earnings belong to (preferred). */
  songSlug?: string;
  /** Slug of the release these earnings belong to (used when no song slug). */
  releaseSlug?: string;
  /** Primary artist slug for grouping. */
  artistSlug: string;
  /** Display label for the song / release (denormalized for statements). */
  title: string;
  platform: RoyaltyPlatform;
  /** Period in YYYY-MM format. */
  periodMonth: string;
  /** Gross amount in the currency below. */
  grossAmount: number;
  currency: string;
  /** Stream / download count for this row, if available. */
  streams?: number;
  notes?: string;
  createdAt: string;
}
...
/**
 * Split configuration for a song or release.
 * Defines what percentage of any earnings a participant receives.
 * The sum of all active splits for a song/release should equal 100.
 */
export interface RoyaltySplit {
  id: string;
  /** Scope: one of song_slug or release_slug must be set. */
  songSlug?: string;
  releaseSlug?: string;
  /** "artist" | "producer" | "other" */
  participantType: "artist" | "producer" | "other";
  /** Slug used to look up name/profile in the CMS (optional for "other"). */
  participantSlug?: string;
  /** Display name (stored for statements even if participant is later deleted). */
  participantName: string;
  /** Percentage of gross earnings (0–100). */
  splitPct: number;
  /** Descriptive role (e.g. "Lead Artist", "Executive Producer"). */
  role?: string;
  createdAt: string;
  updatedAt: string;
}
...
/**
 * A computed payout for one participant for one calendar month.
 * Generated by applying their split % to total earnings for the period.
 * Can be manually marked as paid once payment is sent.
 */
export interface RoyaltyPayout {
  ...
  participantSlug?: string;
  participantName: string;
  /** Period this payout covers, YYYY-MM. */
  periodMonth: string;
  /** Gross earnings attributed to this participant before deductions. */
  grossAmount: number;
  /** Net amount after any deductions (defaults equal to gross if no deductions). */
  netAmount: number;
  currency: string;
  status: PayoutStatus;
...
/**
 * Aggregated monthly statement used in the Statements tab.
 * Derived in-memory from earnings + payouts — not a persisted entity.
 */
export interface RoyaltyMonthlySummary {
  periodMonth: string;
  totalGross: number;
  currency: string;
  earningsByPlatform: Record<string, number>;
  earningsBySong: Array<{ title: string; songSlug?: string; gross: number }>;
