...
export function ThumbnailStudioClient({ initialJobs, producers, generationEnabled }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Job Queue */}
      {studioTab === "queue" && (
        <div className="rounded-2xl border border-white/[0.07] bg-[#08090d] overflow-hidden">

          {/* Mobile/tablet queue toggle bar — hidden on desktop */}
          <div className="flex lg:hidden items-center justify-between border-b border-white/[0.06] px-4 py-2.5 gap-3">
            <p className="text-xs text-white/50 truncate min-w-0">
              {selectedJob
                ? <span className="text-white/80">{selectedJob.title ?? "Untitled"}</span>
                : <span>{jobs.length} jobs in queue</span>
              }
            </p>
            <button
              type="button"
              onClick={() => setShowMobileQueue((p) => !p)}
              className="shrink-0 px-3 py-1.5 rounded-lg border border-white/[0.1] text-[11px] text-white/50 hover:text-white hover:border-white/25 transition-colors"
            >
              {showMobileQueue ? "× Close" : "☰ Jobs"}
            </button>
          </div>

          {/* Mobile queue dropdown */}
          {showMobileQueue && (
            <div className="lg:hidden border-b border-white/[0.06] max-h-56 overflow-y-auto">
              <ThumbnailQueue
                jobs={jobs}
                selectedJobId={selectedJob?.id ?? null}
                onSelect={(j) => { handleSelectJob(j); setShowMobileQueue(false) }}
              />
            </div>
          )}

          {/* Main layout: 3-column on desktop, vertical on mobile */}
          <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-14rem)] lg:overflow-hidden">

            {/* Left panel: Queue — desktop only */}
            <div className="hidden lg:flex w-56 shrink-0 flex-col border-r border-white/[0.06]">
            ...
            <div className="flex-1 min-w-0 flex flex-col border-b lg:border-b-0 lg:border-r border-white/[0.06] lg:overflow-y-auto">
              {loadingProject ? (
            ...
            <div className="w-full lg:w-72 shrink-0 flex flex-col lg:overflow-y-auto">
              {hasJob && project ? (
