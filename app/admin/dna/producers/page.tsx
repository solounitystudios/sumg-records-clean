...
export const metadata = { title: "Producer DNA — SUMG Admin" }
...
export default async function ProducerDNAListPage() {
  await requireAdmin()
  const records = await getDNAByType("producer")
  ...
  return (
    <div className="px-6 py-8 max-w-5xl">
      <div className="mb-8">
        ...
        <h1 className="text-lg font-semibold tracking-tight">Producer DNA</h1>
        <p className="text-xs text-white/35 mt-1">{records.length} record{records.length !== 1 ? "s" : ""}</p>
      </div>

      {records.length === 0 ? (
        <p className="text-sm text-white/30 py-12 text-center">No producer DNA records yet.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {records.map((r) => (
            <Link
              key={r.id}
              href={`/admin/dna/producer/${r.slug}`}
